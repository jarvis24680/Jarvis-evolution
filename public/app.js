...
const PASSKEY_STORAGE = "jarvis:passkey";
...
function GrowthRing({ count }) {
    ...
    return (React.createElement("div", { className: "core-ring", style: { width: 46, height: 46, borderRadius: "50%",
...
const UserIcon = (p) => React.createElement(Icon, { ...p, path: React.createElement(React.Fragment, null,
        React.createElement("path", { d: "M20 21a8 8 0 0 0-16 0" }),
        React.createElement("circle", { cx: "12", cy: "7", r: "4" })) });
const GithubIcon = (p) => React.createElement(Icon, { ...p, path: React.createElement("path", { d: "M9 19c-4.3 1.4-4.3-2.5-6-3m12 5v-3.5c0-1 .1-1.4-.5-2 2.8-.3 5.5-1.4 5.5-6a4.6 4.6 0 0 0-1.3-3.2 4.2 4.2 0 0 0-.1-3.2s-1.1-.3-3.5 1.3a12.3 12.3 0 0 0-6.2 0C6.5 2.8 5.4 3.1 5.4 3.1a4.2 4.2 0 0 0-.1 3.2A4.6 4.6 0 0 0 4 9.5c0 4.6 2.7 5.7 5.5 6-.6.6-.6 1.2-.5 2V21" }) });
function Avatar({ role }) {
    const isUser = role === "user";
    return (React.createElement("div", { className: "msg-avatar", style: {
            width: 26, height: 26, borderRadius: "50%", display: "flex", alignItems: "center",
            justifyContent: "center", flexShrink: 0,
            background: isUser ? "rgba(201,162,39,0.12)" : "rgba(0,242,254,0.12)",
            border: "1px solid " + (isUser ? "rgba(201,162,39,0.35)" : "rgba(0,242,254,0.35)"),
            color: isUser ? "#E8C468" : "#00F2FE",
        } }, isUser ? React.createElement(UserIcon, { size: 13 }) : React.createElement(SparkIcon, { size: 12 })));
}
...
function LockScreen({ onSubmit, error }) {
    const [value, setValue] = useState("");
    return (React.createElement("div", { style: { height: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 } },
        React.createElement("div", { style: { width: "100%", maxWidth: 340, textAlign: "center" } },
            React.createElement("div", { style: { width: 46, height: 46, borderRadius: "50%", margin: "0 auto 18px", border: "1px solid rgba(0,242,254,0.3)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 18px rgba(0,242,254,0.14)" } },
                React.createElement("div", { style: { width: 8, height: 8, borderRadius: "50%", background: "#00F2FE" } })),
            React.createElement("div", { style: { fontFamily: "'Fraunces', serif", fontSize: 21, marginBottom: 6 } }, "Jarvis is locked"),
            React.createElement("div", { style: { fontSize: 13, color: "#8B949E", marginBottom: 18, lineHeight: 1.5 } }, "Enter the passkey to continue."),
            React.createElement("input", { type: "password", value: value, onChange: e => setValue(e.target.value), onKeyDown: e => { if (e.key === "Enter" && value.trim())
                    onSubmit(value.trim()); }, placeholder: "Passkey", style: { width: "100%", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(0,242,254,0.2)", borderRadius: 10, padding: "12px 14px", color: "#F0F6FC", fontSize: 14.5, fontFamily: "'Inter', sans-serif", marginBottom: 12, textAlign: "center" } }),
            error && React.createElement("div", { style: { fontSize: 12.5, color: "#E0A38A", marginBottom: 12 } }, error),
            React.createElement("button", { onClick: () => value.trim() && onSubmit(value.trim()), disabled: !value.trim(), style: { width: "100%", padding: "11px 14px", borderRadius: 10, border: "none", background: value.trim() ? "#00F2FE" : "rgba(0,242,254,0.24)", color: "#07090E", fontSize: 14, cursor: value.trim() ? "pointer" : "default" } }, "Unlock"))));
}

function Jarvis() {
    const [passKey, setPassKey] = useState(() => { try {
        return localStorage.getItem(PASSKEY_STORAGE) || "";
    }
    catch (e) {
        return "";
    } });
    ...
    useEffect(() => {
        if (!passKey)
            return;
        fetch("/api/github/status", { headers: { "x-jarvis-key": passKey } })
    ...
    const sendMessage = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch("/api/chat", {
                method: "POST",
                headers: { "Content-Type": "application/json", "x-jarvis-key": passKey },
            ...
            if (response.status === 401) {
                ...
                setPassKey("");
                setPassError("Incorrect passkey — try again.");
    ...
    if (!passKey) {
        return React.createElement(LockScreen, { onSubmit: unlock, error: passError });
    }
    return (React.createElement("div", { style: { height: "100%", background: "transparent", color: "#F0F6FC", display: "flex", flexDirection: isMobile ? "column" : "row" } },
        React.createElement("aside", { style: {
            ...
            React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 12 } },
                ...
                React.createElement("div", null,
                    React.createElement("div", { className: "brand-title", style: { fontFamily: "'Fraunces', serif", fontSize: 21, fontWeight: 600 } }, "Jarvis"),
            ...
            !isMobile && React.createElement(React.Fragment, null,
                ...
                React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } },
                    ...
                    [{ id: "auto", label: "Auto" }, { id: "simple", label: "Keep it simple" }, { id: "deep", label: "Go deep" }].map(opt => (React.createElement("button", { className: "side-btn", key: opt.id, onClick: () => setDepth(opt.id), style: {
                ...
                React.createElement("div", { style: { height: 1, background: "rgba(0,242,254,0.14)" } }),
                React.createElement("button", { className: "side-btn", onClick: () => setUseWeb(v => !v), style: {
                    ...
                    " Web search ",
                    useWeb ? "on" : "off"),
                voiceSupport.tts && React.createElement("button", { className: "side-btn", onClick: () => { if (voiceOn)
                    ...
                    " Voice replies ",
                    voiceOn ? "on" : "off"),
                React.createElement("button", { className: "side-btn", onClick: () => setShowMemory(v => !v), style: {
                        display: "flex", alignItems: "center", gap: 9, padding: "9px 12px", borderRadius: 8,
        ...
        React.createElement("main", { className: "main-panel", style: { flex: 1, display: "flex", flexDirection: "column", minHeight: 0 } },
            React.createElement("div", { className: "header-bar", style: { height: 56, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", borderBottom: "1px solid rgba(255,255,255,.08)", background: "rgba(13,17,23,.62)", backdropFilter: "blur(18px)" } },
                React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 10, fontSize: 13, color: "#8B949E" } },
                    React.createElement("span", { className: "status-dot", style: { width: 8, height: 8, borderRadius: "50%", background: "#238636", color: "#238636" } }),
                    ...
                    githubStatus?.ok && !isMobile && React.createElement("span", { title: githubStatus.repository ? `Connected to ${githubStatus.repository}` : "GitHub connected", style: { display: "flex", alignItems: "center", gap: 5, fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#8B949E", padding: "3px 7px", border: "1px solid rgba(255,255,255,.1)", borderRadius: 5 } },
                        React.createElement(GithubIcon, { size: 11 }),
                        "REPO LINKED")),
            ...
            React.createElement("div", { ref: scrollRef, className: "scroll", style: { flex: 1, overflowY: "auto", padding: isMobile ? "18px 14px" : "32px 44px", display: "flex", flexDirection: "column", gap: 20 } },
                messages.length === 0 && (React.createElement("div", { style: { margin: "auto", textAlign: "center", maxWidth: 420, color: "#8B949E" } },
                    React.createElement("div", { className: "empty-icon-halo" },
                        React.createElement(CompassIcon, { size: 24, style: { color: "#00F2FE" } })),
                ...
                messages.map((m, i) => (React.createElement("div", { key: i, style: { display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", gap: 9, alignItems: "flex-end" } },
                    m.role !== "user" && React.createElement(Avatar, { role: "assistant" }),
                    React.createElement("div", { className: "msg-bubble " + (m.role === "user" ? "msg-user" : "msg-assistant"), style: {
                            maxWidth: isMobile ? "82%" : "66%", padding: "12px 15px", borderRadius: 14, fontSize: 14.5, lineHeight: 1.6, whiteSpace: "pre-wrap",
                        ...
                        m.role === "assistant" && m.searched && React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 5, fontSize: 10.5, color: "#8B949E", marginBottom: 6, fontFamily: "'IBM Plex Mono', monospace", textTransform: "uppercase" } },
                            ...
                            " searched the web"),
                        m.content),
                    m.role === "user" && React.createElement(Avatar, { role: "user" })))),
                loading && React.createElement("div", { className: "msg-bubble", style: { display: "flex", justifyContent: "flex-start", gap: 9, alignItems: "flex-end" } },
                    React.createElement(Avatar, { role: "assistant" }),
                    React.createElement("div", { style: { padding: "12px 15px", borderRadius: 14, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)", color: "#8B949E", fontSize: 13.5, display: "flex", alignItems: "center", gap: 8 } },
                        ...
                        React.createElement("span", { style: { display: "flex", gap: 4 } },
                            React.createElement("span", { className: "thinking-dot", style: { animationDelay: "0s" } }),
                            React.createElement("span", { className: "thinking-dot", style: { animationDelay: ".15s" } }),
                            React.createElement("span", { className: "thinking-dot", style: { animationDelay: ".3s" } }))))),
            React.createElement("div", { className: "composer-card", style: { padding: isMobile ? "10px 14px 16px" : "14px 44px 24px", borderTop: "1px solid rgba(0,242,254,0.09)" } },
                errorBanner && React.createElement("div", { style: { fontSize: 12.5, color: "#E0A38A", marginBottom: 8 } }, errorBanner),
                isMobile && React.createElement("div", { className: "pill-track", style: { display: "flex", gap: 8, marginBottom: 10, overflowX: "auto" } },
                    React.createElement("button", { className: "pill", onClick: () => setUseWeb(v => !v), style: { display: "flex", alignItems: "center", gap: 5, padding: "6px 10px", borderRadius: 20, border: "1px solid " + (useWeb ? "#00F2FE" : "rgba(201,162,39,0.2)"), background: useWeb ? "rgba(0,242,254,0.09)" : "transparent", color: useWeb ? "#F0F6FC" : "#8B949E", fontSize: 12, whiteSpace: "nowrap" } },
                        React.createElement(GlobeIcon, { size: 12 }),
                        " Web ",
                        useWeb ? "on" : "off"),
                    voiceSupport.tts && React.createElement("button", { className: "pill", onClick: () => setVoiceOn(v => !v), style: { display: "flex", alignItems: "center", gap: 5, padding: "6px 10px", borderRadius: 20, border: "1px solid " + (voiceOn ? "#00F2FE" : "rgba(201,162,39,0.2)"), background: voiceOn ? "rgba(0,242,254,0.09)" : "transparent", color: voiceOn ? "#F0F6FC" : "#8B949E", fontSize: 12, whiteSpace: "nowrap" } },
                        React.createElement(VolIcon, { size: 12 }),
                        " Voice ",
                        voiceOn ? "on" : "off"),
                    ["auto", "simple", "deep"].map(d => (React.createElement("button", { className: "pill", key: d, onClick: () => setDepth(d), style: { padding: "6px 10px", borderRadius: 20, border: "1px solid " + (depth === d ? "#00F2FE" : "rgba(201,162,39,0.2)"), background: depth === d ? "rgba(0,242,254,0.09)" : "transparent", color: depth === d ? "#F0F6FC" : "#8B949E", fontSize: 12, whiteSpace: "nowrap", textTransform: "capitalize" } }, d)))),
                ...
                React.createElement("div", { style: { display: "flex", gap: 10, alignItems: "flex-end" } },
                    ...
                    React.createElement("button", { className: "icon-btn", onClick: () => { var _a; return (_a = fileInputRef.current) === null || _a === void 0 ? void 0 : _a.click(); }, title: "Attach an image", style: { width: 42, height: 42, borderRadius: 10, border: "1px solid " + (attachedImage ? "#00F2FE" : "rgba(201,162,39,0.2)"), background: attachedImage ? "rgba(0,242,254,0.09)" : "rgba(255,255,255,0.04)", color: attachedImage ? "#F0F6FC" : "#8B949E", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 } },
                        React.createElement(ImgIcon, { size: 17 })),
                    React.createElement("textarea", { className: "input-glow", value: input, onChange: e => setInput(e.target.value), onKeyDown: e => { if (e.key === "Enter" && !e.shiftKey) {
                            e.preventDefault();
                            sendMessage();
