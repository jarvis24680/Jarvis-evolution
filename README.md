...
Jarvis is instructed to make the smallest possible diff and never touch
model names, auth checks, or unrelated code as a side effect of a fix —
but it can't fully know which model names or API details have become
outdated since its own training, so when you review a PR, specifically
check for two things: any changed model/version strings, and anything
touching security or auth logic. Those are the two things most likely to
get silently "modernized" back to something wrong.
